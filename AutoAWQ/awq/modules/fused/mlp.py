...
class MixLlamaMLP(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        self.register_buffer('gate_proj_qweight', gate_proj.weight.data)

 
        self.register_buffer('up_proj_qweight', up_proj.weight.data)
        ...
        if isinstance(down_proj, MixLinear_GEMM):
            self.linear = LinearForward
        else:
            raise "no implement"
    def forward(self, x):
