...
class WQLinear_GEMM(nn.Module):
    ...
    def forward(self, x):
        ...
        #print("forward qlinear")
        '''
        import numpy as np
        np.savez("/data/chenyidong/awqllama/input" + str(time.time()),x.squeeze().cpu().numpy())

        import numpy as np
        np.savez("/data/chenyidong/awqllama/output" + str(time.time()) ,out.squeeze().cpu().numpy())
        '''
        #torch.cuda.synchronize()
        #end = time.time()
        #print("kernel time = %.8f"%(end-start))
        #f.writelines("cuda time: %4.8f, compute shape %d, %d \n"%((end-start),out.shape[0],out.shape[1]))
...
class nnLinear_GEMM(nn.Module):
    def __init__(self,  in_features, out_features, bias, dev):
        super().__init__()
        ...
        self.register_buffer('weight', torch.zeros((in_features, out_features // (32 // self.w_bit)), dtype=torch.int32, device=dev))
        if bias:
            self.register_buffer('bias', torch.zeros((out_features), dtype=torch.float16, device=dev))
        else:
            self.bias = None
...
class MixLinear_GEMM(nn.Module):
    ...
    #     import numpy as np
    #     np.savez("/data/chenyidong/mixllama/input" + str(time.time()),x.squeeze().cpu().numpy())


    #     out = torch.mm(inputs,self.weight.T) 

    #     out = out + self.bias if self.bias is not None else out
    #     if self.cnt:

    #         np.savez("/data/chenyidong/mixllama/weight" + str(time.time()) ,self.weight.T.squeeze().cpu().numpy())
    #         self.cnt = 0

        
    #     np.savez("/data/chenyidong/mixllama/output" + str(time.time()) ,out.squeeze().cpu().numpy())
    #     return out.reshape(out_shape)
    @torch.no_grad()
    def forward(self,x):
        #x[x.abs() > 16] = 16
